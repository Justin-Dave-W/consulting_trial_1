declare module "wix-members.v2" {
  ...
  enum Status {
      ...
      BLOCKED = "BLOCKED",
      /** Member is a [guest author](https://support.wix.com/en/article/wix-blog-adding-guest-authors-to-your-blog) for the site blog and cannot log in to the site. */
      OFFLINE = "OFFLINE"
  }
   ...
   * Mutes a member.
   *
   * Muted members can't leave comments, like, and share posts or comments.
   *
   ...
   * Unmutes a member.
   *
   * Unmuted members can leave comments, like, and share posts or comments.
   *
  ...
  /**
   * Changes member status to `APPROVED` and gives access to members-only pages by:
   *
   * - Approving member with the `PENDING` status.
   * - Unblocking member with the `BLOCKED` status.
   * @param _id - ID of the member to unmute.
   * @public
   * @documentationMaturity preview
   * @requiredField _id
   ...
   * Blocks a member.
   *
   * A blocked member can't login to members-only pages.
   *
   ...
   *
   * A disconnected member can't login to a members-only pages, and the member is not visible in the dashboard.
   *
   * >**Note:** This action is irreversible. To connect the same member again, you have to create a member.
  ...
  /**
   * Changes status of multiple members to `APPROVED`, and gives access to members-only pages by:
   *
   * - Approving members with the `PENDING` status by the specified filter.
   * - Unblocking members with the `BLOCKED` status byt the specified filter.
   * @param filter - Query options. See [API Query Language](https://dev.wix.com/docs/rest/articles/getting-started/api-query-language) for more details.
   * @public
   * @documentationMaturity preview
   * @requiredField filter
   ...
   *
   * Blocked members can't login to members-only pages.
   *
   * To unblock multiple members, call Bulks Approve Members.
   * @param filter - Query options. See [API Query Language](https://dev.wix.com/docs/rest/articles/getting-started/api-query-language) for more details.
   * @public
   * @documentationMaturity preview
   * @requiredField filter
