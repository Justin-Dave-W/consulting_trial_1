declare module "wix-crm.v2" {
   ...
   * @param contactId - ID of the contact whose attachment is being uploaded.
   * @param fileName - File name including the extension e.g: `contact-cv.pdf`
   * @public
   * @documentationMaturity preview
   * @requiredField contactId
   ...
   * @returns Contact.
   */
  function createContact(info: ContactInfo$1, options?: CreateContactOptions): Promise<CreateContactResponse>;
   ...
   */
  function updateContact(contactId: string, info: ContactInfo$1, revision: number | null, options?: UpdateContactOptions): Promise<UpdateContactResponse>;
  ...
  /**
   * Deletes multiple contacts based on the specified filter and search criteria.
   *
   * This function returns a Promise that resolves to the `jobId` of the bulk function. When you run the `bulkDeleteContacts()` function, it creates a bulk job. The time required for the bulk job to complete varies based on the number of contacts being deleted. Use the `getBulkJob()` function with the `jobId` to retrieve the status and details of the `bulkDeleteContacts()` job.
   *
   * Errors that might occur while processing the bulk delete contacts job include:
   ...
   * - `CANNOT_DELETE_MEMBER_OWNER_OR_CONTRIBUTOR` - Member cannot be deleted because it is the site owner or a contributor. This can happen only if the request indicated to delete the member.
  ...
  /**
   * Adds and removes labels from multiple contacts based on the specified filter and search criteria.
   *
   * This function returns a Promise that resolves to the `jobId` of the bulk function. When you run the `bulkLabelAndUnlabelContacts()` function, it creates a bulk job. The time required for the bulk job to complete varies based on the number of contacts being labeled and unlabeled. Use the `getBulkJob()` function with the `jobId` to retrieve the status and details of the `bulkLabelAndUnlabelContacts()` job.
   * @internal
   * @documentationMaturity preview
   ...
   * @adminMethod
   */
  function upsertContact(info: ContactInfo$1): Promise<UpsertContactResponse>;
  /**
  ...
  type contactsV4Contact_universal_d_ActivityIcon = ActivityIcon;
  ...
  namespace contactsV4Contact_universal_d {
    export {
      ...
      contactsV4Contact_universal_d_ActivityIcon as ActivityIcon,
