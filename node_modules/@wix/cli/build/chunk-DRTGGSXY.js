...
async function deployGridApp(opts) {
  const deploymentSourceData = await (0, import_variant.match)(opts.source, {
    local: async () => ({
      deploymentData: {
        revision: opts.model.revision,
        content: await loadProjectGridAppContent(opts.model.projectFolder)
      }
    }),
    remote: ({ branch }) => ({
      gitBranch: branch
...
//# sourceMappingURL=chunk-DRTGGSXY.js.map